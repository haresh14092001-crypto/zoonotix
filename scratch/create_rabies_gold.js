...
fs.writeFileSync('scratch/rabies_gold.js', rabiesGold);
console.log('Created scratch/rabies_gold.js');

let buildScript = fs.readFileSync('scratch/build_index_html.js', 'utf8');

const regex = /rabies:\s*\{[\s\S]*?references:\s*\[[^\]]+\]\s*\n\s*\}/;
const newBuildScript = buildScript.replace(regex, rabiesGold);

fs.writeFileSync('scratch/build_index_html.js', newBuildScript);
console.log('Updated scratch/build_index_html.js');

let indexHtml = fs.readFileSync('index.html', 'utf8');
const newIndexHtml = indexHtml.replace(regex, rabiesGold);
