...
function inject(filename) {
  ...
  // Inject CSS if it's html or the html build string
  if (filename.includes('index.html') && !text.includes('.clinical-pearl')) {
    text = text.replace('</style>', styles + '\n</style>');
  }

  // Find start of rabies
  const search1 = "rabies: {";
  const start = text.indexOf(search1);
  // Find nipah which is right after
  const search2 = "nipah: {";
  const end = text.indexOf(search2);
  
  if (start > -1 && end > -1) {
    const before = text.substring(0, start);
    const after = text.substring(end);
    text = before + rabiesGold + "  " + after;
    fs.writeFileSync(filename, text);
    console.log('Successfully injected into ' + filename);
  } else {
    console.log('Failed to find boundaries in ' + filename);
  }
}

inject('scratch/build_index_html.js');
inject('index.html');
inject('scratch/zoonotix_content_enrichment.js');
